    ...
    for d in 1..D
    {
        ...
        for g in 0..ngrids
        {
            ...
            while (l + level_sum) as f64 - (t * l.max(level_max) as f64) <= (n + D as u32 - 1) as f64 - (t * n as f64)  && l.max(level_max) as u32 <= n as u32
            {
                for i in (1..(1 << l)).step_by(2)
                {
                    let is_leaf = l  as u32 + level_sum  as u32 == n as u32 + D as u32 - 1;
...
{
    let boundary_level = boundary_level.unwrap_or(1)  as u8;
    let mut point = GridPoint::new([1; D], [1;D], false);
    let t = T.unwrap_or(0.0); // default to zero (sparse grid)
    let n = levels[0] as u8;
    ...
    for d in 1..D as u32
    {
        ...
        for g in 0..ngrids
        {
            ...
            if ((level_sum as u32 + boundary_level as u32 + num_zero_levels as u32) < (n as u32 + cur_dim)) || (num_zero_levels == cur_dim - 1)
            {
            ...
            let mut upper_bound = if num_zero_levels > 0
            {
                ...
                if n as u32 + cur_dim  < boundary_level as u32 + num_zero_levels
                {
                  continue;
                } else {
                  // upper bound for boundary basis functions
                  (n as u32 + cur_dim - num_zero_levels - boundary_level as u32) as f64
                }
            } 
            else 
            {
                // upper bound for interior basis functions
                (n as u32 + cur_dim - 1) as f64
            };
            ...
            // TODO: This is the one change I've made from the SG++ implementation - allow non-uniform levels. TBD if this works or I need to revert this section.
            let n = levels[d] as u32;
            while (l + level_sum) as f64 - (t * l.max(level_max) as f64) <= upper_bound && l.max(level_max) as u32 <= n as u32
            {
                for i in (1..(1 << l)).step_by(2)
                {
