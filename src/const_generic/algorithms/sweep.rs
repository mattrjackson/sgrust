use crate::{const_generic::{iterators::grid_iterator::{GridIteratorT, HashMapGridIterator}, storage::SparseGridData}, errors::SGError};

pub trait SweepFunction<const D: usize, T, R>
{
    fn execute_in_place(&mut self, values: &mut [R], iterator: &mut HashMapGridIterator<D>, storage: &SparseGridData<D>, dimension: usize) -> Result<(), SGError>;
}
...
pub(crate) fn sweep_1d_boundary_in_place<const D: usize, T, R, F: SweepFunction<D, T, R>>(function: &mut F, storage: &SparseGridData<D>, values: &mut [R],
    dim_sweep: usize) -> Result<(), SGError>
{
    let mut dim_list = vec![0; D-1];
    let mut idx = 0;
    for i in 0..D
